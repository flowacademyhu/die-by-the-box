...
const move_a = (player, tomb_ami_a_map) => {
  const Vertikalis = player.posy;
  const Horizontalis = player.posx;
  const felette = tomb_ami_a_map[Vertikalis - 1][Horizontalis];
  const allasa = player.head;
  const nezese = player.facing;
  const mapmagassaga = tomb_ami_a_map.length;
  // nem a szélén van
  if (player.posx !== 0) {
...
const move_d = (player, tomb_ami_a_map) => {
  ...
  const nezese = player.facing;
  const mapmagassaga = tomb_ami_a_map.length;
  const mapVege = tomb_ami_a_map[1].length - 1;
  // nem a szélén van
